...
export class EditGroupComponent implements OnInit {
  ...
  searchById() {
    this.activatedRoute.params
      .pipe(switchMap(({ id }) => this.bookService.getDetailById(id)))
      .subscribe({
        next: (resp) => {
          this.editId = resp.id;
          this.newGrpForm.reset({
            spot: `${resp.destination}`,
            iDate: this.formattedDate(resp.departureDate),
            fDate: this.formattedDate(resp.returnDate),
            budget: `${resp.budget}`,
            people: `${resp.minimumNumberOfMembers}`,
            interest: '',
            plan: `${resp.itinerary}`,
          });
        },
  ...
  formattedDate(inDate: string): string {
  ...
  onSubmit(): void {
    ...
    this.bookService.updateGroup(this.editId, this.body).subscribe({
      next: () => {
        this.newGrpForm.reset();
